...
Decompressor.prototype.decompress = function decompress(buffer) {
  this.log.trace({ buffer: buffer }, 'Starting header decompression');
  ...
  this.log.debug({ headers: headers }, 'Header decompression is done');
  return headers;
};
...
Compressor.prototype.compress = function compress(headers) {
  this.log.debug({ headers: headers }, 'Starting header compression');
  var i;
  ...
  this.log.trace({ buffer: buffer }, 'Header compression is done');
  return buffer;
};
