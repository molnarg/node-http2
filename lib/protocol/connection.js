...
Connection.prototype._receive = function _receive(frame, done) {
  ...
  if (this._goneawayLocal && !(frame.stream in this._streamIds)) {
...
Connection.prototype._onFinish = function _onFinish() {
  for (var id in this._streamIds) {
    this._streamIds[id].upstream.end();
  }
  this._closed = true;
  this.push(null);
};
...
Connection.prototype._onStreamCountChange2 = function _onStreamCountChange2(change) {
  ...
  if (this._closing && (this._activeStreamCount === 0) && !this._sendScheduled) {
    this._log.debug({}, 'No more active streams, closing underlying connection stream.');
    this._closing = false;
    this._closed = true;
    this.push(null);
  }
