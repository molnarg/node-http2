...
function Endpoint(log, role, settings, bootstrapping, filters) {
  Duplex.call(this);
...
// * Initializing logging infrastructure
  ...
  //   initialization.
  this._initializeDataFlow(role, settings, bootstrapping, filters || {});
...
Endpoint.prototype._writePrelude = function _writePrelude() {
  this._log.debug({ prelude: CLIENT_PRELUDE }, 'Sending the client connection header prelude.');
...
Endpoint.prototype._initializeDataFlow = function _initializeDataFlow(role, settings,  bootstrapping, filters) {
  ...
  this._connection   = new Connection(this._log, firstStreamId, settings, bootstrapping);
