...
//   - **message.cancel**: cancels the response, and emits 'close'
//
...
Server.prototype._start = function _start(socket) {
  ...
  // Note that we do not automatically end the endpoint when the socket is closed.
  // That is handled below by listening to 'end' and 'error' directly.
  endpoint.pipe(socket).pipe(endpoint, { end: false });
  ...
  // Errors are not piped through, so we need to take some action
  socket.once('error', function(err) {
    endpoint.notifySocketClosed(err ? 'PROTOCOL_ERROR' : null);
  });

  socket.once('end', function() {
    // This means we got a TCP FIN (half-closed)
    endpoint.notifySocketClosed();
  });
...
OutgoingResponse.prototype._onStateChange = function onStateChange(state) {
  if (!this._finished) {
    ...
    if (closed && !this._closed) {
      // We went into a closed state without finishing
      this._closed = true;
      this.emit('close');
    }
  }
...
OutgoingResponse.prototype.writeHead = function writeHead(statusCode, reasonPhrase, headers) {
  if (this.headersSent || this._closed) {
    return;
  }
...
function IncomingResponse(stream) {
  ...
  this._closed = false;
  stream.on('state', this._onStateChange.bind(this));
}
IncomingResponse.prototype = Object.create(IncomingMessage.prototype, { constructor: { value: IncomingResponse } });

IncomingResponse.prototype.cancel = function() {
  this.stream.reset('CANCEL');
};

IncomingResponse.prototype._onStateChange = function(state) {
  var closed = (state === 'CLOSED');
  if (!this._closed && closed && !this.stream.ended) {
    this.emit('close');
    this._closed = true;
  }
};
