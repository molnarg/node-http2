...
// copied from https://github.com/brianloveswords/base64url to avoid this modules dependencies
...
function Server(options) {
  ...
  // store HTTP2 upgrade information
  //
  this._bootstrapping = {};
  ...
  //
  if (options.upgrade) {
	this._log.debug({ upgrade: options.upgrade}, 'Server starting HTTP2 upgrade.');
	this._server = http.createServer();
	// this._start.bind(this);
	self=this;

	this._server.on('upgrade', function(request, socket, header) {
	  self._log.debug({ upgrade: options.upgrade}, 'HTTP/2 server receiving client HTTP1.1 Connection Upgrade.');
	
	   if (request.headers['http2-settings'] && request.headers.upgrade && request.headers.connection ) {
		var http2_settings = new Buffer(toBase64(request.headers['http2-settings']), 'base64');
		socket.write('HTTP/1.1 101 Switching Protocols\r\n' +
	  	...
	  	self._log.debug({ method: request.method, url: request.url,  }, 'Storing the HTTP1.1 request for HTTP2 Stream1.');

		var headersFiltered = {};
		Object.keys(request.headers).map(function(hdr) {
			if (hdr != 'http2-settings' && hdr != 'upgrade' && hdr != 'connection' && hdr != 'host') {
				headersFiltered[hdr] = request.headers[hdr];
			};
		});
		...
		upgradeRequest.headers[':method'] = request.method;
		upgradeRequest.headers[':scheme'] = 'HTTP/1.1';
		upgradeRequest.headers[':host'] = request.headers.host;
		upgradeRequest.headers[':path'] = request.url;
		self._bootstrapping._upgradeRequest=upgradeRequest;
  		self._start(socket);

        } else {

    		throw new Error('HTTP1.1 Upgrade does not include Http2-Settings field.');
		this.close();
	}
    });
 ...
 else if ((options.key && options.cert) || options.pfx) {
  ...
  // HTTP2 over plain TCP
    ...
    this._log.error('Trying to create HTTP/1 server ?');
   throw new Error('HTTP1.1 server not supported.');
...
Server.prototype._start = function _start(socket) {
  var endpoint = new Endpoint(this._log, 'SERVER', this._settings, this._bootstrapping);
...
Agent.prototype.request = function request(options, callback) {
  ...
  if (!options.plain && !options.upgrade && options.protocol === 'http:') {
    this._log.error('Trying to send a HTTP1.1 request ?');
    throw new Error('HTTP1.1 is not supported.');
  }
  ...
  // * HTTP/2 over HTTP1.1 Upgrade
  else if (options.upgrade) {
	var buffers = [];
	Serializer.SETTINGS({ settings: options.settings }, buffers);
	this._log.debug({ draft:implementedVersion, settings: options.settings }, 'Starting Upgrade of HTTP1 connection to HTTP2');
	...
	var query_options = {
	    ...
	    headers: {
	      ...
	      ,'Upgrade': implementedVersion 
	      ,'Http2-Settings': fromBase64(Buffer(buffers[0]).toString('base64'))
	      ,'User-Agent': 'Emile HTTP2 Upgrade in Gabor code'
	    }
	};
	var query = http.get(query_options);

	var request = new OutgoingRequest(this._log);
	this.bootstrapping.request=request;
	var self = this;
	query.on('upgrade', function(res, socket, upgradeHead) {
		self._log.debug({upgrade: res.headers.upgrade}, 'Client got upgraded!');
...
OutgoingRequest.prototype._start = function _start(stream, options) {
  ...
  this._log = stream._log.child({ component: 'http' });
  this._log.debug(options, 'Start processing an Outgoing Request.');
