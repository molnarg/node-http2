...
//   - has two subclasses for easier interface description: **ServerRequest** and **ClientResponse**
// - **Class: http2.ServerRequest (IncomingMessage)**
...
// - **Class: http2.ClientResponse (IncomingMessage)**
...
// - **Class: http2.PushRequest (ServerRequest)**
...
Server.prototype._start = function _start(socket) {
  ...
  endpoint.on('stream', function _onStream(stream) {
    var response = new ServerResponse(endpoint, stream, logger);
...
function ServerResponse(endpoint, stream, log) {
...
ServerResponse.prototype._implicitHeaders = function _implicitHeaders() {
  if (!this.headersSent) {
...
ServerResponse.prototype._onRequestHeaders = function _onRequestHeaders(headers) {
  this._requestHeaders = headers;
};

ServerResponse.prototype.push = function push(options) {
  if (!this.headersSent) {
  ...
  return new ServerResponse(this.endpoint, pushStream, this._log);
};
...
ClientResponse.prototype._onPromise = function _onPromise(stream, headers) {
  var dummyRequestStream = new Readable();
  dummyRequestStream._read = function noop() {};
  var request = new ServerRequest(dummyRequestStream, this._log);
  dummyRequestStream.emit('headers', headers);
  dummyRequestStream.push(null);

  var response = new ClientResponse(stream, this._log);
