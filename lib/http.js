...
//
// - **Class: http2.Server**
//   - **Event: 'connection'**: there's a second argument (`endpoint`: a reference to the [Endpoint]
//     (endpoint.html) object tied to the socket) if the negotiation of HTTP/2 was successful.
...
//   - **Event: 'socket'**: it's not emitted in case of an HTTP/2 incoming message.
//   - **Event: 'stream'**: in case of an HTTP/2 incoming message, a reference to the associated
//     [HTTP/2 Stream](stream.html) object is emitted.
...
//   - **Event: 'push' (IncomingPromise)**: signals the intention of a server push. If there's no
//     listener for this event, then push streams will always be cancelled.
...
//   - **Event: 'response' (IncomingResponse)**: signals the arrival of the actual push stream.
