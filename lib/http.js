...
function IncomingMessage(stream) {
  ...
  // * Forwarding error and close event from the stream
  stream.on('error', this.emit.bind('error'));
  stream.on('close', this.emit.bind('close'));
}
...
function OutgoingResponse(stream) {
  ...
  // * Forwarding error and close event from the stream
  this.stream.on('error', this.emit.bind('error'));
  this.stream.on('close', this.emit.bind('close'));
}
