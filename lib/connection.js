...
Connection.prototype._initializeStreamManagement = function _initializeStreamManagement(firstStreamId) {
  ...
  //
  //     Streams that are not allowed to send data are not present in this set. There are two
  //     possible reason for this:
  //     * the stream cannot be opened because it would be over limit (MAX_CONCURRENT_STREAMS)
  //     * the stream has been promised but the PUSH_PROMISE is not sent yet
  this._streamsIds = [];
...
// Creating a new (inbound or outbound) stream with the given `id` means
...
// Creating an *inbound* stream with the given id
Connection.prototype._incomingStream = function _incomingStream(id) {
  ...
  this.emit('incoming_stream', stream, id);
  return stream;
};
...
Connection.prototype.createStream = function createStream() {
  ...
  // * When it becomes closed, we free the given ID.
  function tryToActivate() {
    // If blocked or already activated, then there's nothing to do
    if (blocked || activated) {
      return;
    }

    // If there's enough slot, then do the activation
    if (self._streamCount < self._streamLimit) {
      self._activateStream(stream);
      activated = true;
    }

    // Else try again when slots free up
    else {
      self.once('stream_slot_change', tryToActivate);
    }
  }

  stream.on('promise_initiated', function() {
    blocked = true;
  });

  stream.on('promise_sent', function() {
    blocked = false;
    tryToActivate();
  });
  ...
  stream.on('state', function(state) {
    ...
    var was_active = (active_states.indexOf(stream.state) !== -1);
...
Connection.prototype._read = function _read() {
  ...
  var more_needed = true, stream, stream_id, frame;
  for (var i = 0; i < this._streamPriorities.length && more_needed; i++) {
    stream = this._streamPriorities[i];
    stream_id = this.getIdOf(stream);
