...
// Allocating a stream ID with the appropriate parity, or return `null` if it would be over stream
// count limit.
//
// Note that this is not in sync with the [HTTP/2 spec][1], since it only counts streams in certain
// states to allocate a slot.
// [1]: http://tools.ietf.org/html/draft-ietf-httpbis-http2-04#section-5.1.2
Connection.prototype._allocateStream = function _allocateStream() {
  if (this._streamCount < this._streamLimit) {
...
// Freeing the given stream ID
Connection.prototype._freeStream = function _freeStream(id) {
  this._streamCount -= 1;
  if (this._streamCount === this._streamLimit - 1) {
    this.emit('free_stream_slot');
  }
};

// Changing the stream count limit
Connection.prototype._updateStreamLimit = function _updateStreamLimit(newStreamLimit) {
  var previousStreamLimit = this._streamLimit;
  this._streamLimit = newStreamLimit;
  if (this._streamCount >= previousStreamLimit && this._streamCount < newStreamLimit) {
    this.emit('free_stream_slot');
  }
};
...
Connection.prototype.createStream = function createStream() {
  var stream, id = this._allocateStream();

  // * If we reached the stream limit already, the caller should retry when there's free slot (the
  //   'free_stream_slot' event is emitted)
  if (id === null) {
    this._log.trace({ streamCount: this._streamCount, streamLimit: this._streamLimit },
                    'Could not create new outbound stream because of the stream count limit');
    stream = null;
  }

  // * Otherwise creating a new Stream. When it becomes closed, we free the given ID.
  else {
    stream = this._newStream(id);
...
Connection.prototype._write = function write(frame, encoding, done) {
  ...
  if (!stream) {
    ...
    this.emit('incoming_stream', stream, frame.stream);
...
Connection.prototype._initializeLifecycleManagement = function _initializeLifecycleManagement() {
  ...
  this.on('incoming_stream', function(stream, id) {
