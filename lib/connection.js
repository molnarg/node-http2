...
Connection.prototype._addStream = function _addStream(id, stream) {
...
Connection.prototype._initializeControl = function _initializeControl() {
  ...
  this._addStream(0, { upstream: this._control });
...
Connection.prototype._write = function write(frame, encoding, done) {
  ...
  if (!stream) {
    this._addStream(frame.stream, new Stream(this._log.child({ stream: frame.stream })));
  }
