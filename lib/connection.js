...
Connection.prototype._initializeStreamManagement = function _initializeStreamManagement(firstStreamId) {
  ...
  //   * `_streamsOut` is an ordered set of the streams that are allowed to send frames. The order
...
Connection.prototype._freeStreamSlot = function _freeStreamSlot() {
  this._streamCount -= 1;
  this.emit('stream_slot_change');
};

Connection.prototype._allocateStreamSlot = function _allocateStreamSlot(callback) {
  if (this._streamCount < this._streamLimit) {
    this._streamCount += 1;
    callback();
  } else {
    this.once('stream_slot_change', this._allocateStreamSlot.bind(this, callback));
  }
};
...
Connection.prototype.createStream = function createStream() {
  ...
  var activate = this._enableSending.bind(this, stream);
  var allocateAndActivate = this._allocateStreamSlot.bind(this, activate);
  stream.once('active', function() {
    if (promisePending) {
      stream.once('promise_sent', allocateAndActivate);
    } else {
      allocateAndActivate();
    }
  });
  ...
  stream.once('inactive', this._freeStreamSlot.bind(this));
