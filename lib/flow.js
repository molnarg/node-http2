...
// * **push(frame): bool**: schedules `frame` for sending. Returns `true` if it needs more frames
//   in the output queue, `false` if the output queue is full, and null if did push the frame into
//   the output queue (instead, it pushed it into the flow control queue).
// * **_push(frame, remainderCallback): bool**: like push, but never puts the frame into the
//   flow control queue. Instead, it pushes directly into the output queue if possible (according
//   to the flow control window) and calls `remainderCallback` with the remaining non-pushable part
//   of the frame. It is capable of dividing DATA frames into multiple chunks. Use this instead
//   of `push` if you always want to have empty flow control queue (but never mix the two).
