...
function Flow(flowControlId) {
  ...
  this._queue.unshift = this._queue.unshift.bind(this._queue);
  this._queue.push = this._queue.push.bind(this._queue);
...
Flow.prototype._read = function _read() {
  ...
  //   the extent it is possible with respect to the window size and output queue feedback)
    ...
    while (moreNeeded && (frame = this._queue.shift())) {
      moreNeeded = this._push(frame, this._queue.unshift);
    }
...
Flow.prototype.read = function read(limit) {
  ...
  var frame = Duplex.prototype.read.call(this);
...
// `_push(frame)` It pushes `frame` into the output queue and decreases the flow control window
// size. It is capable of splitting DATA frames into smaller parts, if the window size is not enough
// to push the whole frame. It calls `remainderCallback` synchronously with the frame it was not
// able to push to the output queue. The remainder may be the whole frame or the remaining part of a
// DATA frame. The return value is similar to `push` except that it returns `null` if it did not
// push anything to the output queue.

Flow.prototype._push = function _push(frame, remainderCallback) {
  var forwardable = undefined;
  var remainder = undefined;

  if ((frame === null) || (frame.type !== 'DATA') || (frame.data.length <= this._window)) {
    forwardable = frame;
  }
    ...
    forwardable = {
      ...
      type: 'DATA',
      flags: {},
      data: frame.data.slice(0, this._window)
    };
    frame.data = frame.data.slice(this._window);
    remainder = frame;
  }
...
Flow.prototype.push = function push(frame) {
  ...
  if (this._queue.length === 0) {
    return this._push(frame, this._queue.push);
  } else {
