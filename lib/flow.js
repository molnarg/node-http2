...
function Flow(log, flowControlId) {
  Duplex.call(this, { objectMode: true });

  this._log = (log || logging.root).child({ component: 'flow' });
...
Flow.prototype._forwardable = function _forwardable(frame) {
  return (frame.type !== 'DATA') || (this._window >= frame.data.length);
};
...
Flow.prototype.push = function push(frame) {
  ...
  //console.log('push', this._queue.length, this.wouldForward(frame), frame);
