...
Stream.prototype._initializeManagement = function _initializeManagement() {
  this.upstream.on('receiving', function(frame) {
    if (frame.type === 'PUSH_PROMISE') {
      this.emit('promised', frame.headers, frame.promised_stream);
...
Stream.prototype._transition = function transition(sending, frame) {
  ...
  if (!error && frame.type === 'PUSH_PROMISE') {
    assert(frame.promised_stream.state === 'OPEN');
...
// `promised` is called when this stream is promised by one of the endpoints of the connection.
// If it's us, `sending` is true, when it's the peer, then `sending` is false.
//
// * Sending a PUSH_PROMISE frame marks the associated stream (this) for later use. The stream state
//   for the reserved stream transitions to "reserved (local)".
// * Receiving a PUSH_PROMISE frame marks the associated stream (this) as reserved by the remote
//   peer. The state of the stream becomes "reserved (remote)".
Stream.prototype.promised = function promised(sending, headers, parent) {
  assert(this.state === 'IDLE');
  this.emit('promise_initiated');
