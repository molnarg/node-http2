...
//
// * **initiated**: boolean that is true if this stream was initiated by us
//
...
Stream.prototype._transition = function transition(sending, frame) {
  ...
  if (PUSH_PROMISE && !error) {
    ...
    frame.promised_stream.initiated = sending;
  }

  // Signaling how sending this frame changes the active stream count (-1, 0 or +1)
  // Active stream count can be changed by incoming frames too: when an initiated stream is closed
  // by the peer. This is checked separately in the parent connection.
  if (sending && this.initiated) {
    frame.count_change = (activeState(this.state) - activeState(initialState));
    this._log.trace({ frame: frame, change: frame.count_change }, 'Active count change.');
  }
