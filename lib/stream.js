...
// * **setInitialWindow(size)**: when creating the stream, the initial flow control window size
...
// Handling a write request from the user:
...
// flag on the last frame. If there's no frame in the queue, then we create a 0 length DATA frame.
// We could do this all the time, but putting the flag on an existing frame is a nice optimization.
var endFrame = {
  type: 'DATA',
