...
function Stream(log) {
  ...
  this._upstream = new Duplex();
  this._read = this._upstream._read = function noop() {};
  this._upstream._write = function(frame, encoding, done) {
...
Stream.prototype._transition = function transition(sending, frame) {
  ...
  if (error) {
    if (sending) {
      this.log.error({ frame: frame, state: this.state }, 'Stream error: sending illegal frame.');
      throw new Error('Sending illegal frame (' + frame.type + ') in ' + this.state + ' state.');
    } else {
      this.log.error({ frame: frame, state: this.state }, 'Stream error: received illegal frame.');
