...
// Public events, methods and fields, with arguments listed in the numbered list:
//
// * **Event: 'headers'**: signals an incoming HEADERS frame
//   1. a map of the header names and values
// * **Event: 'promise'**: signals an incoming PUSH_PROMISE frame
//   1. a map of the promise header names and values
//   2. a reference to the promised stream
// * **Event: 'error'**: signals an error
//   1. upper cased type of the error (PROTOCOL_ERROR, etc.)
// * **headers(headers, [priority])**: send a HEADERS frame
//   1. a map of the header names and values
//   2. priority value to be sent (optional)
// * **promise(headers, stream)**: promise a stream
//   1. a map of the promise header names and values
//   2. a reference to the promised stream
// * **reset(error)**: reset the stream with an error code
//   1. the name of the error
// * **setInitialWindowSize(size)**: when creating the stream, the initial flow control window size
//   is 65535. It can be changed *any time* ([as described in the standard][2]) using this method.
//   1. the new value of the initial window size
// * **upstream**: a duplex stream that is used by the parent connection to write/read frames
...
// [2]: http://tools.ietf.org/html/draft-ietf-httpbis-http2-04#section-6.9.2
...
Stream.prototype._initializeFlowControl = function _initializeFlowControl() {
  ...
  this._initialWindowSize = this._window = INITIAL_WINDOW_SIZE;
...
// Flow control window size is manipulated using the `_increaseFlowControlWindow` method.
//
...
// stream.  For streams, the sender sends a RST_STREAM with the error code of FLOW_CONTROL_ERROR
// code.

var WINDOW_SIZE_LIMIT = Math.pow(2, 31) - 1;

Stream.prototype._increaseWindow = function _increaseWindow(size) {
...
//
// Flow control can be disabled externally by calling setInitialWindowSize with `Infinity`.

Stream.prototype.setInitialWindowSize = function setInitialWindowSize(initialWindowSize) {
  this._increaseWindow(initialWindowSize - this._initialWindowSize);
