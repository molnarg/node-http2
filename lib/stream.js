...
// The main aspects of managing the stream are:
	function Stream(log) {
	  ...
	  // * logging
  this._log = log.child({ component: 'stream', s: this, id: this.id });
...
Stream.prototype._pushUpstream = function _pushUpstream(frame) {
  this._log.debug(frame, 'push Upstream');
  if ( this._stream1hdr == true ) { 
  	this._stream1hdr == false
  	this._log.debug(frame, 'push Upstream: Frame HEADER of Stream1 not sent');
  } else {
