...
Serializer.prototype._transform = function _transform(frame, encoding, done) {
  if (!(frame.type in Serializer)) throw new Error('Unknown frame type: ' + frame.type)
...
Serializer.SETTINGS = function writeSettings(frame, buffers) {
  ...
  if (settings_left.length !== 0) throw new Error('Unknown settings: ' + settings_left.join(', '))
...
Deserializer.SETTINGS = function readSettings(buffer, frame) {
  ...
  if (buffer.length % 8 !== 0) throw new Error('Invalid SETTINGS frame.')
  for (var i = 0; i < buffer.length / 8; i++) {
    ...
    if (!setting || setting.name in frame.settings) continue
...
Deserializer.PING = function readPing(buffer, frame) {
  if (buffer.length !== 8) throw new Error('Invalid size PING frame.')
  frame.data = buffer
}
