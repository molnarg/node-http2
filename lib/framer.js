...
Serializer.prototype._transform = function _transform(frame, encoding, done) {
  this.log.debug({ frame: frame }, 'Outgoing frame');
      ...
      if (this._waiting_for_header) {
        ...
        if (this._frame.type) {
          try {
            ...
            this.log.debug({ frame: this._frame }, 'Incoming frame');
            this.push(this._frame);
          } catch(error) {
            this.log.error({ state: this, error: error }, 'Incoming frame parsing error');
            this.emit('error', error);
          }
        } else {
          this.log.warn({ frame: this._frame }, 'Unknown type incoming frame');
        }
        this._next(COMMON_HEADER_SIZE);
      }
    }
