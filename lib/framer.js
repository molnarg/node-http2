...
Serializer.prototype._transform = function _transform(frame, encoding, done) {
  ...
  for (var i = 0; i < buffers.length; i++) this.push(buffers[i])
  done()
}
...
Deserializer.prototype._next = function(size) {
  ...
  if (this._waiting_for_header) this._frame = {}
}
...
Serializer.commonHeader = function writeCommonHeader(frame, buffers) {
  ...
  if (size > 65535) throw new Error('Too large frame: ' + size + ' bytes')
...
Serializer['HEADERS+PRIORITY'] = function writeHeadersPriority(frame, buffers) {
...
Deserializer['HEADERS+PRIORITY'] = function readHeadersPriority(buffer, frame) {
...
Deserializer['SETTINGS'] = function readSettings(buffer, frame) {
  ...
  for (var i = 0; i < buffer.length / 8; i++) {
    ...
    if (setting.name in frame.settings) continue
...
Serializer['PING'] = function writePing(frame, buffers) {
  if (!frame.data || frame.data.length !== 8) throw new Error('PING frames must carry an 8 byte payload.')
  buffers.push(frame.data)
}
...
Serializer['HEADERS'] = function writeHeaders(frame, buffers) {
  buffers.push(frame.data)
}

Deserializer['HEADERS'] = function readHeaders(buffer, frame) {
  frame.data = buffer
}
