...
Serializer.prototype = Object.create(stream.Transform, { constructor: { value: Serializer } })
...
Deserializer.prototype = Object.create(stream.Transform, { constructor: { value: Serializer } })
...
Serializer.prototype.commonHeader = function writeCommonHeader(frame) {
  ...
  // Length
  if (payload_size > 65535) throw new Error('Too large frame: ' + frame.length + ' bytes (maximal size: 65535 bytes)')
...
/*
  ...
  The HEADERS+PRIORITY frame is identical to the HEADERS frame (Section 3.8.9), preceded by a single reserved bit and
  a 31-bit priority; see Section 3.4.2.

  HEADERS+PRIORITY uses the same flags as the HEADERS frame, except that a HEADERS+PRIORITY frame with a CONTINUES bit
  MUST be followed by another HEADERS+PRIORITY frame. See HEADERS frame (Section 3.8.9) for any flags.
*/
...
Serializer.prototype['HEADERS+PRIORITY'] = function writeHeadersPriority(frame) {
  ...
  // Priority
  if (frame.priority > 0x7fffffff) throw new Error('Too large priority value: ' + frame.stream)
...
/*
  ...
  The RST_STREAM frame contains a single unsigned, 32-bit integer identifying the error code (Section 3.5.3). The error
  code indicates why the stream is being terminated.
...
/*
  ...
  The SETTINGS frame defines the following flag:
    ...
    CLEAR_PERSISTED (0x2):
    Bit 2 being set indicates a request to clear any previously persisted settings before processing the settings.
*/
...
/*
  ...
  The PUSH_PROMISE frame (type=0x5) is used to notify the peer endpoint in advance of streams the sender intends to
  initiate. The PUSH_PROMISE frame includes the unsigned 31-bit identifier of the stream the endpoint plans to create
  along with a minimal set of headers that provide additional context for the stream.
  ...
  PUSH_PROMISE uses the same flags as the HEADERS frame, except that a PUSH_PROMISE frame with a CONTINUES bit MUST be
  followed by another PUSH_PROMISE frame. See HEADERS frame (Section 3.8.9) for any flags.
*/
...
/*
  ...
  The PING frame (type=0x6) is a mechanism for measuring a minimal round-trip time from the sender, as well as
  determining whether an idle connection is still functional.
...
Serializer.prototype['PING'] = function writePing(frame) {
  if (!frame.data || frame.data.length !== 8) throw new Error('PING frames must carry an 8 byte length payload.')
  return frame.data
}
...
/*
 ...
 The last stream identifier in the GOAWAY frame contains the highest numbered stream identifier for which the sender of
 the GOAWAY frame has received frames on and might have taken some action on.
...
/*
  ...
  Additional type-specific flags for the HEADERS frame are:
    ...
    CONTINUES (0x2):
    The CONTINUES bit indicates that this frame does not contain the entire payload necessary to provide a complete
    set of headers.
...
/*
  ...
  The following additional flags are defined for the WINDOW_UPDATE frame:
    ...
    Bit 2 being set indicates that flow control for the identified stream or connection has been ended; subsequent
    frames do not need to be flow controlled.

  The payload of a WINDOW_UPDATE frame is a 32-bit value indicating the additional number of bytes that the sender can
  transmit in addition to the existing flow control window. The legal range for this field is 1 to 2^31 - 1 (0x7fffffff)
  bytes; the most significant bit of this value is reserved.
*/
...
// The least significant bit (0x1) - the FINAL bit - is defined for all frame types as an indication that this frame
// is the last the endpoint will send for the identified stream.
